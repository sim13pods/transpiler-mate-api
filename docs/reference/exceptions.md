...
| `context.get_processes_by_type(...)` finds no match and `fail_if_empty=True` | `PluginExecutionError` is raised by the method | The requested process type is unavailable in the input document or selected IDs. |
